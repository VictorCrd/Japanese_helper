...
from dev_tests.JLPT_LVL_script import split_japanese, get_top_10_words, top_10
...
def helper():
    ...
    if request.method == "POST":
        if "nm" in request.form:
            ...
            _top_10 = get_top_10_words(df_1, df_2, df_3, df_4, df_5)
            _top_10['button'] = np.NaN
            for i in range(0,10):
        ...
        if "youtube" in request.form:
            ...
            top_10 = pd.concat([df_5, df_4, df_3, df_2, df_1], ignore_index=True).sort_values('Count', ascending=False).reset_index(drop=True)[0:10]
            top_10['Known word'] = np.NaN
            for i in range(0,10):
                top_10['Known word'][i] = '<a href="#" name="%d" id="add"><button class="btn-small waves-effect waves-light blue darken-1" type="submit" title="Add this word to known words, this word wont be shown next time you encouter it">Add</button></a>' %(i)

            return render_template("helper.html", top_10_words=top_10.to_html(escape=False),
                                   text=text, CORE_N5=CORE_N5, CORE_N4=CORE_N4, CORE_N3=CORE_N3, CORE_N2=CORE_N2, CORE_N1=CORE_N1, mimetype='text/html', df_5=df_5.to_html(),
